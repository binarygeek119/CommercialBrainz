...
async def register(data: UserCreate, db: AsyncSession = Depends(get_db)):
    ...
    if invite_only:
        if not data.invite_code or not data.invite_code.strip():
            raise HTTPException(
    status_code=403,
     detail="Registration is invite-only. An invite code is required.")
        try:
...
async def login(data: UserLogin, db: AsyncSession = Depends(get_db)):
    ...
    return Token(access_token=create_access_token(
        user.id, remember_me=data.remember_me))
...
async def verify_email(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=str(e)) from e
    return MessageResponse(
    message="Email verified. You can vote and submit edits.")
...
async def submission_quiz(user: User = Depends(get_current_user)):
    if user_can_submit(user):
     ...
     detail="You already have submission access")
    return {
...
async def submission_upgrade(
    ...
    if not user_email_verified(user):
        raise HTTPException(
    status_code=403,
     detail="Verify your email address before upgrading.")
...
@router.get("/deletion-request",
     response_model=AccountDeletionRequestPublic | None)
...
@router.post("/deletion-request",
    response_model=AccountDeletionRequestPublic,
     status_code=status.HTTP_201_CREATED)
