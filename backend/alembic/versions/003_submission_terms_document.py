...
# Inline seed — mirrors app/data/submission_terms_v1.py
_TERMS = {
    "version": 1,
...
def upgrade() -> None:
    ...
    op.create_index("ix_submission_terms_documents_is_active", "submission_terms_documents", ["is_active"])

    op.add_column("users", sa.Column("submission_terms_version", sa.Integer(), nullable=True))

    doc_id = str(uuid.uuid4())
    sections_json = json.dumps(_TERMS["sections"])
    op.execute(
        sa.text(
            """
            INSERT INTO submission_terms_documents (id, version, title, intro, sections, is_active)
            VALUES (:id, :version, :title, :intro, CAST(:sections AS jsonb), true)
            """
        ).bindparams(
            id=doc_id,
...
def downgrade() -> None:
    op.drop_column("users", "submission_terms_version")
    op.drop_index("ix_submission_terms_documents_is_active", table_name="submission_terms_documents")
    op.drop_table("submission_terms_documents")
