...
`main` and `google` are legacy aliases; prefer `testing` and `cloudflare`.

## Workflow

1. Open PRs against **`testing`** (not `cloudflare`).
2. Merge into `testing` → CI → auto-deploy to **`commercialbrainz-vm`** (DuckDNS testing).
3. When testing looks good, promote **`testing` → `cloudflare`** → auto-deploy to **`commercialbrainz-public`** (public).
...
Or open a GitHub PR from `testing` into `cloudflare`.
