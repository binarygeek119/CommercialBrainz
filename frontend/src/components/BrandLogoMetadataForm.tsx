...
function toFormState(logo: AdvertiserLogo): FormState {
  return {
    label: logo.label ?? "",
...
function toPayload(form: FormState): AdvertiserLogoSubmit {
...
export default function BrandLogoMetadataForm({ advertiserSbid, logo, onSubmitted }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const edit = await api.submitAdvertiserLogoMetadata(advertiserSbid, logo.id, toPayload(form));
      setResult(edit);
      onSubmitted?.();
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginTop: "0.75rem" }}>
      <div className="form-group">
        <label htmlFor={`logo-label-${logo.id}`}>Version label</label>
        <input
          id={`logo-label-${logo.id}`}
          value={form.label}
      ...
      <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
        <div className="form-group" style={{ flex: "1 1 120px" }}>
          <label htmlFor={`logo-year-${logo.id}`}>Year</label>
          <input
            id={`logo-year-${logo.id}`}
            type="number"
            min={1800}
            max={2100}
            value={form.year}
        ...
        <div className="form-group" style={{ flex: "1 1 160px" }}>
          <label htmlFor={`logo-month-${logo.id}`}>Month (optional)</label>
          <select
            id={`logo-month-${logo.id}`}
            value={form.month}
      ...
      <div className="form-group">
        <label htmlFor={`logo-event-${logo.id}`}>Event / occasion</label>
        <input
          id={`logo-event-${logo.id}`}
          value={form.event}
      ...
      <div className="form-group">
        <label htmlFor={`logo-notes-${logo.id}`}>Notes (optional)</label>
        <textarea
          id={`logo-notes-${logo.id}`}
          rows={2}
          value={form.notes}
