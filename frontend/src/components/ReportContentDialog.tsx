...
export type ReportTargetType = "commercial" | "brand";
...
export default function ReportContentDialog({
  ...
  const noun = targetType === "brand" ? "brand" : "commercial";
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      if (targetType === "brand") {
        await api.reportBrand(targetSbid, payload);
      } else {
        await api.reportCommercial(targetSbid, payload);
      }
      setDone(true);
      onSubmitted?.();
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="report-dialog-card">
        <h2 id="report-content-title" className="report-dialog-title">
          Report {noun}
        </h2>
